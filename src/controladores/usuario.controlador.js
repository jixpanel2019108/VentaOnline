...
function loginFacturas(req,res){
    var params = req.body;
    Usuario.findOne({usuario:params.usuario},(err,usuarioEncontrado) => {
        if (err) return res.status(500).send({mensaje: 'Error en la peticion de usuario Usuario'});
        if (usuarioEncontrado){
                ...
                if (passCorrecta){
                    if (params.obtenerToken === 'true'){
                        if(usuarioEncontrado.rol == 'Administrador'){
                            ...
                            Factura.find({usuarioFactura:usuarioEncontrado},(err, facturasEncontradas) => {
                                if (err) return res.status(200).send({mensaje:'Error al hacer la peticion'});
                                if (facturasEncontradas.length == 0){
                                    return res.status(200).send({Sesion:`Bienvenido ${usuarioEncontrado.usuario}`,token: jwt.createToken(usuarioEncontrado), mensaje:"El usuario aun no tiene facturas"});
                                } 
                                return res.status(200).send({Sesion:`Bienvenido ${usuarioEncontrado.usuario}`,token: jwt.createToken(usuarioEncontrado), facturasEncontradas});
                            })
                        }
                    }else{
                        usuarioEncontrado.password = undefined;
